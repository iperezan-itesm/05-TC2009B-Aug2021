...
\begin{table}[htbp]
  \label{Table:MIPS_RType_examples}
    ...
    \code{SRL}  & \code{SRL rd, rs, sa}  & \code{Reg[rd] $\leftarrow$ Reg[rt] >> sa}\\ \hline
    \code{SLA}  & \code{SLA rd, rs, sa}  & \code{Reg[rd] $\leftarrow$ Reg[rt] <<< sa}\\ \hline
    \code{SRA}  & \code{SRA rd, rs, sa}  & \code{Reg[rd] $\leftarrow$ Reg[rt] >>> sa}\\ \hline
    \hline\hline
    \end{tabular}
  \end{table}
\end{frame}
...
\begin{frame}{I-Type store instructions datapath}
  \begin{figure}
  \centering
  \includegraphics[scale=0.68]{non_pipelined_MIPS_Itype_STORE}
  \vspace{-3pt}
